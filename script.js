...
// Optimized Version: prayerList 기반 시스템 - Firebase 표준 구조 활용
// Created: 2025

class FridayPrayerScheduleManager {
  constructor() {
    console.log('FridayPrayerScheduleManager - prayerList 기반 시스템 시작');
  ...
  init() {
    console.log('앱 초기화 시작 - prayerList 기반');
  ...
  setupApplication() {
    ...
    // Firebase 초기화 대기 후 데이터 로드
    this.waitForFirebaseAndLoad();
    
    console.log('앱 설정 완료');
  }

  // Firebase 초기화 대기 및 데이터 로드
    ...
    if (window.firebase && window.firebase.database) {
      ...
      const checkFirebase = setInterval(() => {
        ...
        if (window.firebase && window.firebase.database) {
          ...
          clearInterval(checkFirebase);
          console.warn('Firebase 초기화 실패 - 로컬스토리지에서 데이터 로드');
  ...
  // Handle form submission - prayerList에 직접 추가
  handleFormSubmit(e) {
    e.preventDefault();
  ...
  // Get schedules for current month from prayerList
  ...
  // Save edited schedule - Firebase에서 직접 수정
  saveEditedSchedule() {
  ...
  // Delete schedule - Firebase에서 직접 삭제
  ...
  saveSetlist(date, value) {
    if (this.firebaseReady) {
      this.db.ref('setlists').child(date).set(value);
    } else {
  ...
  saveYoutubeLink(date, value) {
    if (this.firebaseReady) {
      this.db.ref('youtubeLinks').child(date).set(value);
    } else {
  ...
  saveAllExtraInfo(date) {
    ...
    if (this.firebaseReady) {
      const updates = {};
      updates[`setlists/${date}`] = conti;
      updates[`youtubeLinks/${date}`] = youtubeUrl;
...
// Debug function for Firebase connection
window.debugFirebaseConnection = function() {
  console.log('=== Firebase 연결 디버그 ===');
...
/*
=== prayerList 기반 완벽한 시스템 ===

🎯 핵심 특징:
- Firebase prayerList 구조 100% 활용
...
🔧 주요 개선사항:
- prayerList 객체 방식 사용
- Firebase push() 자동 키 생성
- 실시간 리스너로 즉시 업데이트
- 로컬스토리지 백업 시스템
- setlists, youtubeLinks 완전 연동

🚀 운영 방식:
- 새 일정: firebase.database().ref('prayerList').push()
- 수정: firebase.database().ref('prayerList').child(key).update()
- 삭제: firebase.database().ref('prayerList').child(key).remove()
- 실시간: on('value') 리스너로 즉시 화면 업데이트
*/